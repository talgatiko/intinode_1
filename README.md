...
#### 3. Архитектура  
- **Ядро Linux**:  
  - Модуль `intinode.ko` распознаёт специальные файлы по магической строке `intinode ` (первые 8 байт).  
...
#### 4. Формат специального файла  "intinode"
```text  
...
**Результат:**  
- трафик при загрузке  - только используемые файлы, и только те которых нет в локальном кеше.  
- Экономия диска: гигабайты  на контейнер.  
...
**Эффект для облака:**  
- Экономия на хранении данных - контенеры, и виртальные машины.  
